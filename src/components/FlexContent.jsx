...
const Flexcontent = ({
   isHighlight,
   contentType: { heading, title, text, btn, url, img },
}) => {
   return (
      <div
      ...
      >
         ...
         <div className='flex items-center justify-center max-w-xl relative lg:max-w-none w-full'>
            <img
               src={img}
               alt={`img/${heading}`}
               className={`w-auto lg:mt-10 object-fill transitions-theme hover:scale-110 ${
                  isHighlight
                     ? 'h-60 lg:h-56 md:h-52 sm:h-44 xsm:h-36 rotate-6'
                     : 'h-72 lg:h-64 md:h-60 sm:h-48 xsm:h-40 -rotate-6'
               }`}
            />
         </div>
      </div>
